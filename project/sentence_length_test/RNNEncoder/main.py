...
def evaluateTestingPairs(encoder, net, pairs, lang, args):
    ...
    for pair in pairs:
        ...
        if (predict == label):
            ...
            bin_incorrect[label] += 1

    print('Accuracy of the network on the sentence length test set: %d %%' % (
        100 * correct / total))
    for i in range(10):
        if i != 9:
            print("Sentence Length {}-{}: {}/{} (incorrect/total) = {}".format(
                i * 2 + 1, i * 2 + 2, bin_incorrect[i], bin_total[i], bin_incorrect[i] / bin_total[i]))
        else:
            print("Sentence Length 18+: {}/{} (incorrect/total) = {}".format(
                bin_incorrect[i], bin_total[i], bin_incorrect[i] / bin_total[i]))
